var Module = function () {
	...
	var oTable1 = table1.dataTable({
		"aaSorting": [],
		"sAjaxSource": "getTitleList.do?sTempTableParent=.portlet-body",
		"aoColumns": [
			{"mData":"FTitleName"},
			{"mData":"FMonthFen"},
			{
	...
	var modify = function(iRow) {
		...
		$(":input[name=FTitleId]").val(aData.FTitleId);
		$(":input[name=FTitleName]").val(aData.FTitleName);
		$(":input[name=FMonthFen]").val(aData.FMonthFen);		
	};
	var modifyPost = function () {
		wait(el);          
		var url ="titleModify.do" ;
		var queryString = $("#form1").serialize();
		$.ajax({
			url:url ,
			data: queryString,
	...
	var deleteConfirm = function(iRow) {
		confirm("是否确认删除,删除后无法还原?", function(result) {
			if (result) {
				var aData = oTable1.fnGetData(iRow);
				var url ="titleDelete.do";
				var queryString = "FTitleId=" + aData.FTitleId;
				$.ajax({
					url:url ,
					data: queryString,
					dataType: 'json',
